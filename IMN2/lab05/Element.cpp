...
Element::Element(Node *n1, Node *n2, Node *n3, Node *_min, Node *_max)
        : min_global(_min), max_global(_max), S_local(3,3), F_local(3)
{
    nodes.push_back(n1);
    nodes.push_back(n2);
    nodes.push_back(n3);
...
double Element::integrate(int i, int j) {
...
double Element::integrate(int j, double (*ro)(double, double)) {
    double ksi1[]={-0.3333333, -0.0597158717,-0.0597158717, -0.8805682564, -0.7974269853, -0.7974269853, 0.5948539707};
    double ksi2[]={-0.3333333, -0.0597158717,-0.8805682564, -0.0597158717, -0.7974269853,  0.5948539707,-0.7974269853};
    double weights[]={0.45,0.2647883055, 0.2647883055, 0.2647883055, 0.251878361, 0.251878361, 0.251878361};
    ...
    for(int k = 0; k < 7; k++) {
        result += weights[k] * jacobian * fi(j,ksi1[k],ksi2[k]) * ro(x(ksi1[k],ksi2[k]), y(ksi1[k],ksi2[k]));
    }
    return result;
}

void Element::fillLocalMatrix() {
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            S_local(i,j) = integrate(i, j);
        }
    }
...
void Element::fillLocalVector(double (*ro)(double, double)) {
    for (int j = 0; j < 3; ++j) {
        F_local(j) = integrate(j, ro);
    }
...
void Element::fillGlobalMatrix(Eigen::MatrixXd &S) {
    int p, q;
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
...
void Element::fillGlobalVector(Eigen::VectorXd &F) {
    int p, q;
    for (int i = 0; i < 3; ++i) {
        p = nodes[i]->global_n - 1;
        F(p) += F_local(i);
